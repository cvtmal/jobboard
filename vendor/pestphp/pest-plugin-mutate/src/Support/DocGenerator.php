...
class DocGenerator
{
    public static function buildSets(): string
    {
        ...
        return implode("\n\n", array_map(fn (string $set): string => self::buildSet($set), $sets));
    }
...
'.$mutators.'
    ...
    public static function buildMutators(): string
    {
        ...
        usort($mutators, fn (string $a, string $b): int => strnatcmp((string) $a::name(), (string) $b::name()));

        return implode("\n\n", array_map(fn (string $mutator): string => self::buildMutator($mutator), $mutators));
    }
