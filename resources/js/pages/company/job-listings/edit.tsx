...
const breadcrumbs: BreadcrumbItem[] = [
    {
        title: 'Job Listings / Edit Job',
        href: '/company/job-listings',
    },
];

export default function EditJobListing({ auth, jobListing, errors, categoryOptions, companyLogo, companyBanner, currentSubscription }: Props) {
    ...
    const { data, setData, post, processing } = useForm({
        ...
        description_and_requirements: jobListing.description_and_requirements || '',
        benefits: jobListing.benefits || '',
        workload_min: jobListing.workload_min || 80,
        workload_max: jobListing.workload_max || 100,

        // Location information
        workplace: jobListing.workplace || Workplace.ONSITE,
        office_location: jobListing.office_location || auth.company?.city || '',

        // Application details
        employment_type: jobListing.employment_type || CustomEmploymentType.PERMANENT,
        seniority_level: jobListing.seniority_level || SeniorityLevel.MID_LEVEL,

        // Salary information
        salary_min: jobListing.salary_min || '',
        salary_max: jobListing.salary_max || '',
        salary_period: jobListing.salary_period || SalaryPeriod.YEARLY,
        ...
        application_process: jobListing.application_process || ApplicationProcess.EMAIL,
    ...
    const calculateProgress = useCallback(() => {
        let progress = 0;
        if (isStep1Valid()) progress += 25; // Step 1 is 25%
        if (isStep2Valid()) progress += 25; // Step 2 is 25%
        if (completedSteps.includes(3)) progress += 25; // Step 3 is 25%
        if (completedSteps.includes(4)) progress += 25; // Step 4 is 25%
    ...
    const saveDataToLocalStorage = useCallback(() => {
        try {
            localStorage.setItem(`job-listing-edit-draft-${jobListing.id}`, JSON.stringify(data));
    ...
    const goToNextStep = () => {
        ...
        if (current.isFormValid()) {
            ...
            if (currentStep < 4) {
                ...
                // After step 4, save changes and navigate to preview
                handleSaveAndNavigateToPreview();
            }
        }
    };
    ...
    const goToPreviousStep = () => {
        if (currentStep > 1) {
            ...
            localStorage.setItem(`job-listing-edit-current-step-${jobListing.id}`, prevStep.toString());
        }
    };

    const handleSaveAndNavigateToPreview = () => {
        if (!auth.company) {
        ...
        // Update the job listing with current form data
    ...
    const goToStep = (step: number) => {
        ...
        if (step < currentStep || getCurrentStepValidation().isFormValid()) {
            ...
            setCurrentStep(step);
            localStorage.setItem(`job-listing-edit-current-step-${jobListing.id}`, step.toString());
        }
    };
    ...
    const debouncedAutoSave = useCallback(
        debounce(async (formData: any) => {
            try {
                ...
                localStorage.setItem(`job-listing-edit-draft-${jobListing.id}`, JSON.stringify(formData));
                setAutoSaveStatus('saved');
                setLastSavedAt(new Date());
            } catch (error) {
    ...
    useEffect(() => {
        ...
        const draft = localStorage.getItem(`job-listing-edit-draft-${jobListing.id}`);
        if (draft) {
            try {
        ...
        // Load saved step
        const savedStep = localStorage.getItem(`job-listing-edit-current-step-${jobListing.id}`);
        if (savedStep) {
            try {
        ...
        const savedCompletedSteps = localStorage.getItem(`job-listing-edit-completed-steps-${jobListing.id}`);
    ...
    }, [jobListing.id]); // Only depend on jobListing.id
    ...
    const stepperSteps = [
        ...
        },
        {
            ...
            isCompleted: completedSteps.includes(4) || isStep4Valid(),
                    ...
                    <div className="mb-8 space-y-6">
                        <div className="text-center">
                            ...
                            <p className="text-muted-foreground mt-2">
                                Update the {currentStep === 1 ? 'essential' : currentStep === 2 ? 'detailed' : 'optional'} information for your job
                        ...
                        {currentStep === 1 && (
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Update the basic information about your job posting</CardDescription>
                                </CardHeader>
                            ...
                            <div className="space-y-6">
                                ...
                                <Card>
                                    <CardHeader>
                                        ...
                                        <CardDescription>Update how candidates should apply for this position</CardDescription>
                                    </CardHeader>
                                    <CardContent className="space-y-4">
                                        <div>
                                            ...
                                            >
                                                ...
                                                <SelectContent>
                                                    ...
                                                    <SelectItem value={ApplicationProcess.BOTH}>
                                                        <div className="flex items-center gap-2">
                                                            <Mail className="h-4 w-4" />
                                                            <ExternalLink className="h-4 w-4" />
                                                            Both Email & Website
                                        ...
                                        {(data.application_process === ApplicationProcess.EMAIL || data.application_process === ApplicationProcess.BOTH) && (
                                            <div>
                                        ...
                                        {(data.application_process === ApplicationProcess.URL || data.application_process === ApplicationProcess.BOTH) && (
                                            <div>
                            ...
                            <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
                                <div className="text-muted-foreground text-sm">
                                    <div className="font-medium">
                                        ...
                                        {currentStep === 4 && 'Step 4 of 4: Application Process'}
                                    </div>
                                    <div>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <Button
                                        ...
                                        variant="outline"
                                        onClick={() => {
                                            // Navigate to job listing show instead of listing index
                                    ...
                                    {currentStep <= 4 && (
                                        ...
                                        >
                                            {currentStep === 4 ? 'Continue to Preview' : 'Next'}
                                        </Button>
                                    )}
                                </div>
                            </div>
