...
export function filterJobsByCanton(jobs: any[], cantonCodes: string[]): any[] {
    ...
    return jobs.filter((job) => cantonCodes.includes(job.primary_canton_code));
}
...
export function filterJobsBySubRegion(jobs: any[], subRegions: string[]): any[] {
    ...
    return jobs.filter((job) => job.primary_sub_region && subRegions.includes(job.primary_sub_region));
}
